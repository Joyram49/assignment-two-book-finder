...
export default function BookItem({ data, onFavourite }) {
  const { imageUrl, name, id, price, author, rating, isFavourite } = data || {};

  const ratingArray = function (length) {
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='flex items-center justify-center rounded-md border border-[#324251]/30 bg-white p-4'>
        <img
          className='max-w-[144px] w-40 h-36'
          src={imageUrl}
          alt='book name'
        />
      </div>
      ...
      <div className='min-h-56 flex flex-col justify-between'>
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-1'>
            {ratingArray(rating).map((r, index) => (
              <img key={r + index} src={star} />
            ))}
        ...
        <div className='flex items-center gap-3 text-xs lg:text-sm'>
          ...
          <button
            className={`flex min-w-[132px] items-center justify-center gap-1 rounded-md  py-1.5  transition-all  lg:py-1.5 ${
              isFavourite
                ? "bg-[#dfaaaa] text-[#a31242] hover:opacity-80"
